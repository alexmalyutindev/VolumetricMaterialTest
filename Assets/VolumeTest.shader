    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void GetVolumeData(FragInputs fragInputs, float3 V, out float3 scatteringColor, out float density)
            {
                ...
                scatteringColor = 1; //surfaceDescription.BaseColor;
                density = SAMPLE_TEXTURE3D(_MainTex, sampler_MainTex, fragInputs.texCoord0).r; // fragInputs.color.a; // surfaceDescription.Alpha;
            }
            ...
            VertexToFragment Vert(uint instanceId : INSTANCEID_SEMANTIC, uint vertexId : VERTEXID_SEMANTIC)
            {
                ...
                output.positionCS.xy = output.positionCS.xy * _VolumetricMaterialData[_VolumeMaterialDataIndex].
                                                              viewSpaceBounds.zw + _VolumetricMaterialData[
                    _VolumeMaterialDataIndex].viewSpaceBounds.xy;
            ...
            void Frag(VertexToFragment v2f, out float4 outColor : SV_Target0)
            {
                ...
                float3x3 obbFrame = float3x3(_VolumetricMaterialObbRight.xyz, _VolumetricMaterialObbUp.xyz,
